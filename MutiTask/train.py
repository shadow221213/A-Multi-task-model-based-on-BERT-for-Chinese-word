...
def mtl_compute_metrics( eval_pred: EvalPrediction ):
    ...
    # raw_texts = [x['raw_text'] for x in inputs]
    ...
    def visualize_sequence( seg_true, seg_pred, ner_true, ner_pred, seg_label_map, ner_label_map ):
        ...
        for c, t, p in zip(text, seg_true, seg_pred):
            seg_t = seg_inv_map.get(t.item( ), '?')
            seg_p = seg_inv_map.get(p.item( ), '?')
        ...
        for c, t, p in zip(text, ner_true, ner_pred):
            ner_t = ner_inv_map.get(t.item( ), 'O')
            ner_p = ner_inv_map.get(p.item( ), 'O')
            color = 'green' if t == p else 'red'
...
def mtl_train( args, tokenizer, device ):
    ...
    trainer = MultiTaskTrainer(
        ...
        compute_metrics=mtl_compute_metrics,
        callbacks=[
...
if __name__ == '__main__':
    ...
    mtl_train(args, tokenizer, device)
